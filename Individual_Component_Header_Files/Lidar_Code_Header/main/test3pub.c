...
void micro_ros_task_lidar (){
    ...
    //-------------------- setup of node_one, and publisher
	...
	// rcl_init_options_t init_options2 = rcl_get_zero_initialized_init_options();
	// RCCHECK(rcl_init_options_init(&init_options2, allocator2));

#ifdef CONFIG_MICRO_ROS_ESP_XRCE_DDS_MIDDLEWARE
	rmw_init_options_t* rmw_options = rcl_init_options_get_rmw_init_options(&init_options);
	// rmw_init_options_t* rmw_options2 = rcl_init_options_get_rmw_init_options(&init_options2);

	// Static Agent IP and port can be used instead of autodisvery.
	RCCHECK(rmw_uros_options_set_udp_address(CONFIG_MICRO_ROS_AGENT_IP, CONFIG_MICRO_ROS_AGENT_PORT, rmw_options));
	// RCCHECK(rmw_uros_options_set_udp_address(CONFIG_MICRO_ROS_AGENT_IP, CONFIG_MICRO_ROS_AGENT_PORT, rmw_options2));
	...
	RCCHECK(rclc_support_init_with_options(&support, 0, NULL, &init_options, &allocator));
	// RCCHECK(rclc_support_init_with_options(&support2, 0, NULL, &init_options2, &allocator2));
    
    ESP_LOGI(TAG_LIDAR, "create node_one"); //next best thing to do to debug is cross-reference with linorobot and main.c

	// create node_one
	// rcl_node_t node_one;
	RCCHECK(rclc_node_init_default(&node_one, "scanone", "", &support));

    ESP_LOGI(TAG_LIDAR, "create node_two"); //next best thing to do to debug is cross-reference with linorobot and main.c

	// // create node_two
	// // rcl_node_t node_two;
	// RCCHECK(rclc_node_init_default(&node_one, "scantwo", "", &support));
    // ESP_LOGI(TAG_LIDAR, "create pub 1");
	...
	RCCHECK(rclc_publisher_init_default(
		...
		"rover_publisher_lidar_nodeOne_one"));

	// RCCHECK(rclc_publisher_init_default(
	// 	&lidar_publisher,
	// 	&node_one,
	// 	ROSIDL_GET_MSG_TYPE_SUPPORT(sensor_msgs, msg, LaserScan),
	// 	"rover_publisher_lidar_nodeOne_two"));
    ESP_LOGI(TAG_LIDAR, "create pub 2");
	...
	// rcl_timer_t timer2;
	// const unsigned int timer_timeout2 = 1000;
	// RCCHECK(rclc_timer_init_default(
	// 	&timer2,
	// 	&support2,
	...
	// RCCHECK(rclc_executor_add_timer(&executor2, &timer2));
    ...
    // free resources
	RCCHECK(rcl_publisher_fini(&lidar_publisher, &node_one));
	// RCCHECK(rcl_publisher_fini(&lidar_publisher, &node_two));
	RCCHECK(rcl_node_fini(&node_one));
	// RCCHECK(rcl_node_fini(&node_two));
